...
void SoftmaxWithLossLayer<Dtype>::LayerSetUp(
  ...
  string type_ = this->layer_param_.precision().rounding();*/
...
void SoftmaxWithLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      //yi * log(pi) -> log(pi) if yi=1
      loss -= log(std::max(prob_data[i * dim + label_value * inner_num_ + j],
                           Dtype(FLT_MIN)));
      ++count;
    }
  }
  Dtype lossNormalized = loss / get_normalizer(normalization_, count);
  
  
  top[0]->mutable_cpu_data()[0] = lossNormalized;
  if (top.size() == 2) {
...
void SoftmaxWithLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    caffe_scal(prob_.count(), loss_weight, bottom_diff);
     ...
     /*if(fixedPrecision_) {
		RoundFixedPoint(this->layer_param_, bottom[0]->mutable_cpu_diff(),bottom[0]->count(),NULL);
