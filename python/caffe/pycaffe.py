...
def _Net_forward(self, blobs=None, start=None, end=None, **kwargs):
    ...
    #unpacked = {out: self.blobs[out].data for out in outputs}
    for out in outputs:
	unpacked[out] = self.blobs[out].data
    return unpacked
...
def _Net_backward(self, diffs=None, start=None, end=None, **kwargs):
    ...
    for out in outputs:
	unpacked_diff[out] = self.blobs[out].diff
    return unpacked_diff
...
def _Net_forward_all(self, blobs=None, **kwargs):
    ...
    for out in set(self.outputs + (blobs or [])):
	all_outs[out] = []
...
def _Net_forward_backward_all(self, blobs=None, diffs=None, **kwargs):
    ...
    for diff in set(self.outputs + (diffs or [])):
	all_diffs[diff] = []
	...
	if in_ in kwargs:
		batch1[in_] = kwargs[in_]

    forward_batches = self._batch(batch1)

    for out in self.outputs:
	if out in kwargs:	
		batch2[out] = kwargs[out]

    backward_batches = self._batch(batch2) 
...
def _Net_batch(self, blobs):
    ...
    for b in range(num_batches):
        i = b * batch_size
	for name in blobs:
		toyield[name] = blobs[name][i:i + batch_size]
	yield toyield
